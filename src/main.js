...
let hasTriggeredParam13Liquid = false; // 🌟 追蹤 Param13 特級高潮液體是否已觸發過
...
function createTreatmentUI() {
  ...
  btnMedicine.addEventListener('click', (e) => {
    ...
    targetParam14 = 0;             // 完全恢復正常
    swipeCounterForSwelling = 0;    // 清除重置15次計數器陣列
...
function updateParams() {
  ...
  if (targetParam10 === 0) {
    ...
    localSwipeCount = 0;
    param8PressCount = 0; 
    hasTriggeredParam13Liquid = false; // 完美重置
    isPipActive = false;
    pipManuallyClosed = false;
  }
  ...
  // 🌟【Param13 核心重構控制陣列】：動態融合遮罩公式與第25次噴水液體劫持
  ...
  // 動態平滑注入 Param14 參數至 Live2D 模型核心
